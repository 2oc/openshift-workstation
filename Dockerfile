...
dnf -y install vim telnet sl wget xz bzip2 zip tar curl iputils php php-cli php-pear nmap nmap-ncat tmux fish zsh htop python-pip git nc libffi gcc libffi-devel perl-Perl-Critic-More.noarch python-devel automake gcc gcc-c++ kernel-devel cmake redhat-rpm-config openssl-devel vim bind-utils glibc-common glibc-langpack-en procps-ng git libjpeg-turbo-devel python-devel python3-devel ncurses-devel fish man-db man libtool rsync mosh openssh-clients openssh-server autojump-fish libevent-devel libevent && \
